...
// Take a request to save and image and pass it along to the appropriate image.
//
// This method does not support a Progress Bar
//

  public boolean saveImage(int imageNum, String path, String prefix, boolean silent)
  {
	int 			tvRoll;
	String			tvStatus;
	boolean			tvWrite,tvProceed;
	int				tvOverWriteInput;
...
// cause an image object to be created and stored in the roll.
	...
	if(ivImageRoll[tvRoll] == null)
	{
		...
		if(!silent)
		{
			System.out.println("Reading from camera... " + ivImageRoll[tvRoll].getSaveName());		
		}
		
		ivCamera.transferImageData(ivImageRoll[tvRoll]);
	}
	
// With the image object created, tell the image to save itself to the computer. Note that an existing file is overwritten!
	...
	if(tvStatus == "OK")
	{
		if(!silent)
		{
			System.out.println("Save Completed... " + ivImageRoll[tvRoll].getSaveName());		
		}			
	} else
	{
		if(!silent)
		{
			System.out.println(tvStatus);	// Some issue was encountered - e.g. Write Access denied for selected save location - so bail out		
			tvProceed = false;
		}		
	}
...
// With the image object created, tell the image to save itself to the computer. Then update progress bar.
  ...
  public boolean saveAllImages(String path, String prefix, boolean silent)
  {
	int 			tvRoll, i, j;
	LockEventMgr	tvLockEventMgr;
	String			tvStatus;
	boolean			tvWriteCurrent, tvWriteFuture, tvProceed;
	int				tvOverWriteInput;
...
// With the image object created, tell the image to save itself to the computer. Existing files will be overwritten!
		...
		if(tvStatus == "OK")
		{
			...
			if(!silent)
			{
				System.out.println(i + " of " + j + " Saves Completed... " + ivImageRoll[tvRoll].getSaveName());	
			}		
